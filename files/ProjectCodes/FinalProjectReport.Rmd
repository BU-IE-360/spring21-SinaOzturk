...
In this part, the prediction results for the period between 12th of June, 2021 and 26th of June, 2021 for each product will be summarized. Every prediction was made by using the data until 2 days before the prediction day. That is, for example, in 14th of June, the predictions were made for 15th of June using the data until 13th of June. Prediction results for the 9 products are shown below:

```{r, warning=FALSE}
predictions <- read_excel("pred.xlsx")
predictions
```
...
- We follow for each product different approach according to its characteristic and the data volume. With enough data, we try to build ARIMA or ARIMAX models with weekly decomposition. For small data sets, we have to construct LM models with regressors. To develop further, for the external regressors and significant attributes in LM models, we try to predict these regressors also. With building different LM models for each regressors, we also want to have more accurate predictions.
...
[Product ID: 31515569](files/ProjectCodes/item4.R)

[Product ID: 6676673](files/ProjectCodes/item5.R) 

[Product ID: 7061886](files/ProjectCodes/item6.R) 

[Product ID: 85004](files/ProjectCodes/item7.R) 

[Product ID: 4066298](files/ProjectCodes/item8.R) 

[Product ID: 32939029](files/ProjectCodes/item9.R) 

[Product ID: 48740784, 73318567, 32737302](files/ProjectCodes/item123.html) 
